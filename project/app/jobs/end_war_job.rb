class EndWarJob < ApplicationJob
	...
	def send_winner(war, winner)
		war.guilds.each do |guild|
			guild.members.each do |member|
				member.send_notification("War with #{war.opponent(guild).name} just ended ! #{winner.name} won #{war.prize}!", "/warindex", "war")
			end
		end
	end
	...
	def send_tie(war)
		war.guilds.each do |guild|
			guild.members.each do |member|
				member.send_notification("War with #{war.opponent(guild).name} just ended ! It's a tie! Prize was split into two!", "/warindex", "war")
			end
		end
	end
end
