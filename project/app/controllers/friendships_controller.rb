class FriendshipsController < ApplicationController
	...
	def accept
		...
		@user.send_notification("#{@current_user.name} accepted your friend request", "/user/#{@user.id}", "friend_request")
	end
	...
	def refuse
		...
		@request = FriendRequest.where(requestor: @user, receiver: @current_user)
		@request.destroy
		@user.send_notification("#{@current_user.name} refused friend request", "/user/#{@user.id}", "friend_request")
	end
