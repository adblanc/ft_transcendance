...
export default class CanvaView extends ModalView<Rectangle> {
  ...
  collide_player_one(player_one) {
    if (
      player_one.paddle.x < this.ball.right() &&
      player_one.paddle.x + 15 > this.ball.left() &&
      this.ball.y > player_one.paddle.y &&
      this.ball.y < player_one.paddle.y + player_one.paddle.height
    ) {
  ...
  collide_player_two(player_two) {
    if (
      this.ball.left() < this.player_two.paddle.x + 15 &&
      player_two.paddle.x < this.ball.right() &&
      this.ball.y > player_two.paddle.y &&
      this.ball.y < player_two.paddle.y + player_two.paddle.height
    ) {
      this.ball.velocity.x *= -1.05;
    }
  }
  ...
  update(dt): Number {
    ...
    if (
      this.ball.left() < this.player_two.paddle.x + 15 ||
      this.ball.right() > this.player_one.paddle.x
    ) {
